...
export default function ModalWindow({ open, onClose, clickedElement, clickedDay, month, year, setIsOpen , handleOpen}) {
 // if(!isSureModalOpen) return null
 if (!open) return null;
  const portalElement = document.getElementById('portal')

  const {isSureModalOpen, handleCloseSureModal, handleOpenSureModal} = useSureModal()
      ...
      <Modal open={open}>
        <ModalWrapper>
          ...
          <BookedTime>
            ...
            {oneUser && oneUser.getTableInfo.timeForBooking.length==0 ? <NothingToShowText>Nothing to show</NothingToShowText> : <></>}
          </BookedTime>
          <ModalButton
           onClick={handleBook}
          //onClick={handleClick}
